  ...
  <div>
    <el-form ref="form" label-width="80px" :model="spu">
      ...
      <el-form-item label="SPU图片">
        ...
        file-list 照片墙需要展示的数据【数据：数组里面的元素务必要有name、url属性】
        -->
        <el-upload
...
export default {
  ...
  data() {
    return {
      ...
      //添加SPU，如果是添加SPU的时候并没有向服务器发请求，数据收集到SPU，收集数据的时候有哪些字段呀，看文档
      spu: {
        category3Id: 0,
        //描述
        description: "",
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      ...
      //对于服务器而言，不需要name、url字段，将来对于有的图片提交给服务器的时候，需要处理的
    ...
    async initSpuData(spu) {
      ...
      if (spuImageResult.code == 200) {
        ...
        //由于照片墙显示的数据需要数组，数组里面的元素需要有name与url字段
        //需要把服务器返回的数据进行修改
    ...
    addSaleAttrValue(row){
      //点击销售属性值当中的添加按钮的时候，需要由button变为input，通过当前销售属性的inputVisible控制
