  ...
  methods: {
    ...
    addAttrValue(){
      ...
      this.attrInfo.attrValueList.push({
        ...
        valueName:'',
        flag:true,      //flag属性：给每一个属性值添加一个标记flag，用户切换查看模式与编辑模式，好处，每一个属性值可以控制自己的模式切换，该属性是响应式数据(数据变化视图跟着变化)
      });
      this.$nextTick(()=>{
    ...
    updateAttr(row){
      ...
      this.attrInfo.attrValueList.forEach(item => {
        //这样写也可以给属性值添加flag属性，但是会发现试图不会跟着变化(因为flag不是响应式数据)
    ...
    toLook(row){
      ...
      //当前的编辑模式切换为查看模式【让input消失，显示span】
      row.flag = false;
    },
    //点击span的回调，变为编辑模式
    toEdit(row,index){
    ...
    async addOrUpdateAttr(){
      //整理参数：1.如果用户添加很多属性值，切属性值为空的不应该提交给服务器
