<template>
  <div>
    ...
    <!-- 
      ...
      layout：可以实现分页器布局
      pager-count:按钮的数量  如果设9，则连续页码数为7
    ...
    <el-dialog :title="tmForm.id?'修改品牌':'添加品牌'" :visible.sync="dialogFormVisible">
      <!-- form表单 :model属性，这是属性的作用是，把表单的数据收集到哪个对象的身上，将来表单验证，也许需要这个属性 -->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      //表单验证规则
      //required:必须要校验字段(前面五角星有关系)   message:提示信息   trigger:用户行为设置(事件的设置：blur、change)
      rules:{
        tmName: [
  ...
  methods: {
    ...
    async getPageList(pager = 1) {
      ...
      //获取品牌列表的接口
      //当你向服务器发请求的时候，这个函数需要带参数，因此老师在data当中初始化两个参数，带给服务器传递参数
